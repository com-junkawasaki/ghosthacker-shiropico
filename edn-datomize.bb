...
;; (adapted for gftdcojp/ai-gftd-ghosthacker-shiropico from com-junkawasaki/root's
;;  manifest/edn-datomize.bb — this child repo has no manifest/ dir, schema.edn
;;  lives at repo root instead. Also adds a `wrap-vec` mode for files whose
;;  top-level content is already a vector-of-maps (e.g. shotlist files) rather
;;  than a single top-level map.)
;;
...
;; 名前空間を付けた属性名にリネームする。既にベクタ・オブ・マップ（各要素が
;; 1 entity 相当）のファイルは各要素に :db/id -1,-2,... を振り、同じ名前空間
;; 規則でキーをリネームする（wrap-vec）。値が Datomic の scalar valueType
...
;; ---------- wrap-vec (top-level vector-of-maps, e.g. shotlist files) ----------
;;
;; Some files are already shaped as a homogeneous vector of "record" maps
;; (one map per shot/step/entry) with bare, un-namespaced keys. This is much
;; closer to real tx-data than a single top-level map — rather than collapse
;; it into one blob entity (which would destroy per-record queryability), each
;; element becomes its own entity: assign :db/id -1,-2,-3,... and namespace
;; the bare keys the same way wrap-map does.

(defn already-tx-data-vec?
  [content]
...
(defn wrap-vec! [rel-path ns-name]
  (let [f (io/file root rel-path)
    ...
    (cond
      ...
      :else
      (let [entities (map-indexed (fn [i m] (entity-from-record m ns-name i)) content)
            attrs (distinct (mapcat #(schema-attrs % ns-name) content))]
  ...
  (let [[mode a b] args]
    (case mode
      ...
      "adr-file" (let [report (atom {:ok [] :skipped [] :errors [] :attrs []})]
                   ...
                   (merge-schema! (:attrs @report))
                   (println @report))
      (do (println "usage: bb edn-datomize.bb [wrap-map <path> <ns> | wrap-vec <path> <ns> | adr-dir <dir> | adr-file <path>]")
          (System/exit 1)))))
