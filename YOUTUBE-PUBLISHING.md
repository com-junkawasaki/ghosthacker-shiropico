...
```sh
kbb --backend sci tools/youtube-oauth.cljk \
...
3. Upload the video as `unlisted` with `kbb --backend sci tools/youtube-publish.cljk`
   (the API client is `kotoba-lang/com-youtube`, pinned in `nbb.edn`).
4. Attach the matching SRT track with `kbb --backend sci tools/youtube-attach-captions.cljk`.
5. Verify playback, audio, captions, title, description, and Shorts framing.
6. Change visibility with `kbb --backend sci tools/youtube-set-privacy.cljk --privacy public
