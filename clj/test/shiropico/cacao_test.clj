...
(deftest graph-cid-format
  (testing "CIDv1/dag-cbor/sha2-256 graph handle is base32 'bafyrei…'"
    ;; Same input string shape as kotobase.cid-test's graph-cid-format
    ;; (CLJS) — if this JVM port is byte-identical to the kotobase.net edge,
    ;; it MUST produce the exact same 59-char CID for the exact same name.
...
(deftest per-actor-key-persists
  (testing "load-or-create! generates once, then reloads the same identity"
    (let [p  (str (System/getProperty "java.io.tmpdir") "/shiropico-id-test-" (System/nanoTime) ".edn")
...
(deftest resources-encode-capability-and-graph
  (is (= ["kotoba://op/datom:read" "kotoba://graph/shiropico"]
         (c/grant->resources {:cap :cap/read :scope "shiropico"})))
